...
(function (window, $) {
    ...
    var ngMoveSelectionHandler = function ($scope, elm, evt, grid) {
        ...
        var charCode = evt.which || evt.keyCode,
            ...
            lastInRow = false,
            firstInRow = false,
            rowIndex = $scope.selectionProvider.lastClickedRow === undefined ? 1 : $scope.selectionProvider.lastClickedRow.entity.Index,
    ...
    var ngAggregate = function (aggEntity, rowFactory, rowHeight, groupInitState) {
        ...
        this.aggrselected = aggEntity.aggrselected;
        	...
        	for (var i = 0; i < newRows.length; i++) {
        		if (!$scope.renderedRows[i] || (newRows[i].isAggRow || $scope.renderedRows[i].isAggRow)) {
        			...
        			if (!newRows[i].isAggRow) {
        				...
        				$scope.renderedRows[i].aggrselected = newRows[i].aggrselected;
        			}

        			$scope.renderedRows[i].selected = newRows[i].entity.Selected;
        		} else {
        			$scope.renderedRows[i].setVars(newRows[i]);
        		}

        		$scope.renderedRows[i].rowIndex = newRows[i].entity.Index;
    ...
    var ngRow = function (entity, config, selectionProvider, rowIndex, $utils) {
        ...
        this.rowIndex = entity.Index;
        this.utils = $utils;
        this.selected = entity.Selected;
        // always use the default cursor, not a pointer as requested by femsa
        this.cursor = this.config.enableRowSelection ? 'default' : 'default';
    ...
    ngRow.prototype.ensureEntity = function (expected) {
        if (this.entity !== expected) {
            ...
            this.selected = this.entity.Selected;
        }
    };
    ...
    ngRow.prototype.alternatingRowClass = function () {
        ...
        var classes = {
            'ngRow': true,
            'selected': this.selected && this.entity.IsSelectable,
            'even': isEven,
            'odd': !isEven,
    ...
    ngRow.prototype.copy = function () {
    	this.clone = new ngRow(this.entity, this.config, this.selectionProvider, this.entity.Index, this.utils);
    ...
    ngRow.prototype.setVars = function (fromRow) {
        ...
        this.selected = fromRow.entity.Selected;
        this.orig = fromRow;
    };
    var ngRowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
        ...
        self.buildEntityRow = function (entity, rowIndex) {
        	return new ngRow(entity, self.rowConfig, self.selectionProvider, entity.Index, $utils);
        };
            ...
            if (g.values) {
                ...
                for (var prop in g) {
                    if (prop === NG_FIELD || prop === NG_DEPTH || prop === NG_COLUMN) {
                        ...
                        var j = 0;
                        var childrens = g[prop].values.length;
                        for (var i = childrens; i--;) {
                        	g[prop].values[i].entity.Selected ? j += 1 : 0;
                        }
                        agg.aggrselected = (j == childrens ? true : false);
        ...
        var searchEntireRow = function (condition, item, fieldMap) {
            ...
            for (var prop in item) {
                if (item.hasOwnProperty(prop)) {
                    ...
                    var pVal = item[prop];
                    if (typeof pVal === 'object' && pVal !== null && pVal !== undefined) {
            ...
            if (searchConditions.length === 0) {
            	...
            	grid.filteredRows = grid.rowCache.filter(function (row) {
            		if (row.entity.ContactId) {
            			searchConditions[0].column = 'FullName';
            			searchConditions[0].columnDisplay = 'fullname';
            		}
    ...
    var ngSelectionProvider = function (grid, $scope, $parse) {
        ...
        self.ChangeSelection = function (rowItem, evt) {
            ...
            if (evt && evt.shiftKey && !evt.keyCode && self.multi && grid.config.enableRowSelection) {
                if (self.lastClickedRow) {
                    ...
                    var thisIndx = rowItem.entity.Index;
                    ...
                    self.lastClickedRowIndex = rowItem.entity.Index;
        ...
        self.setSelection = function (rowItem, isSelected) {
            if (grid.config.enableRowSelection && rowItem.entity.IsSelectable) {
                if (!isSelected) {
                ...
                rowItem.entity.Selected = isSelected;
                if (rowItem.orig) {
        ...
        self.toggleSelectAll = function (checkAll, bypass, selectFiltered) {
        	var rows = grid.rowFactory.parsedData;
        	if (bypass || grid.config.beforeSelectionChange(rows, checkAll)) {
        		...
        		for (var i = 0; i < rows.length; i++) {
                	if (rows[i].isAggRow) {
                		rows[i].aggrselected = checkAll;
                	} else {
                		rows[i].selected = checkAll;
                		rows[i].entity.Selected = checkAll;
                	}
                	if (checkAll) {
        		...
        		// Organization's checkbox wasn't changing when this function was executed, so we render the grid again.
                grid.rowFactory.renderedChange();
            }
        };
    };
                            ...
                            options.selectRow = function (rowIndex, state) {
                                if (grid.rowCache[rowIndex]) {
                                    if (grid.rowCache[rowIndex].clone) {
                                    	...
                                    	grid.rowCache[rowIndex].clone.entity.Selected = state ? true : false;
                                    }
                                    grid.rowCache[rowIndex].setSelection(state ? true : false);
                                    grid.rowCache[rowIndex].entity.Selected = state ? true : false;
                                }
                            };
    ...
    angular.module("ngGrid").run(["$templateCache", function ($templateCache) {
        ...
        $templateCache.put("checkboxCellTemplate.html",
          "<div class=\"ngSelectionCell\"><input tabindex=\"-1\" ng-show=\"row.entity.IsSelectable\" class=\"ngSelectionCheckbox\" type=\"checkbox\" ng-checked=\"row.selected\" /></div>"
        );
        ...
        $templateCache.put("footerTemplate.html",
          ...
          "            <span class=\"ngLabel\">Total: {{maxRows()}}</span>" +
          "        </div>" +
          ...
          "            <span style=\"float: left; margin-top: 3px;\" class=\"ngLabel\"></span>" +
              // The class selectvisible is used to show the first element of the pagers because otherwise it displays as hidden
          "            <select ui-select2=\"{ dropdownCssClass: 'selectvisible'}\" style=\"float: left;height: 27px; width: 100px\" ng-model=\"pagingOptions.pageSize\" >" +
          ...
          "            <input class=\"ngPagerCurrent\" min=\"1\" max=\"{{maxPages()}}\" type=\"text\" style=\"width:50px; height: 24px; margin-top: 1px; padding: 0 4px;\" ng-model=\"pagingOptions.currentPage\"/>" +
        ...
        $templateCache.put("gridTemplate.html",
          "<div ng-class=\"{'ui-widget-header':jqueryUITheme, 'ui-corner-top': jqueryUITheme}\" ng-style=\"topPanelStyle()\">" +
        ...
        $templateCache.put("headerCellTemplate.html",
          ...
          "    <div ng-click=\"col.sort($event)\" ng-class=\"'colt' + col.index\" class=\"ngHeaderText\"><div>{{col.displayName}}</div></div>" +
        ...
        $templateCache.put("headerRowTemplate.html",
          ...
          "	<div class=\"ngVerticalBar\" ng-style=\"{height: col.headerRowHeight}\">&nbsp;</div>" +
