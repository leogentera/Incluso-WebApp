    ...
    link: function(scope, element, attrs, ctrls) {
      ...
      function refill( updateSelected ) {
        ...
        if ( ngModel.$modelValue ) {
          ...
          if (isNaN(date)) {
          	var splitDateTime = ngModel.$modelValue.split('T')
          	if (splitDateTime.length != 0) {
          		var splitDate = splitDateTime[0].split('-')
          		date = new Date(splitDate[1] + "-" + splitDate[2] + "-" + splitDate[0] + " " + splitDateTime[1]);
          	}
          	if (isNaN(date)) {
          		valid = false;
  ...
  .directive('modalWindow', ['$timeout', function ($timeout) {
    return {
      ...
      link: function (scope, element, attrs) {
          ...
          scope.winh = attrs.winh || '';
          scope.winw = attrs.winw || '';

          if (attrs.winh) {
              element.find(".modal-body.contacts").css("height", attrs.winh - 110);
          }
  ...
  .factory('$modalStack', ['$document', '$compile', '$rootScope', '$$stackedMap',
    function ($document, $compile, $rootScope, $$stackedMap) {
      ...
      $modalStack.open = function (modalInstance, modal) {
        ...
        angularDomEl.attr('winw', modal.winw);
        angularDomEl.attr('winh', modal.winh);
          ...
          $modal.open = function (modalOptions) {
            ...
            templateAndResolvePromise.then(function resolveSuccess(tplAndVars) {
              ...
              $modalStack.open(modalInstance, {
                ...
                winh: modalOptions.winh,
                winw: modalOptions.winw
              });
...
angular.module("template/datepicker/datepicker.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/datepicker/datepicker.html",
    ...
    "      <th colspan=\"{{rows[0].length + showWeekNumbers}}\"><button type=\"button\" class=\"btn pull-left\" ng-click=\"move(-1)\"><i class=\"icon-chevron-left prev\"></i></button><button type=\"button\" class=\"btn btn-block\" ng-click=\"toggleMode()\"><strong>{{title}}</strong></button><button type=\"button\" class=\"btn pull-right\" ng-click=\"move(1)\"><i class=\"icon-chevron-right next\"></i></button></th>\n" +
    "    </tr>\n" +
...
angular.module("template/modal/window.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/modal/window.html",
    "<div class=\"modal fade {{ windowClass }}\" ng-class=\"{in: animate}\" ng-style=\"{height: winh+'px', width: winw+'px', 'z-index': 1050 + index*10}\" ng-transclude></div>");
}]);
